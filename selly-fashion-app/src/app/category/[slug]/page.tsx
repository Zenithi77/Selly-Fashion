...
export default function CategoryPage() {
  ...
  const [category, setCategory] = useState<ClothingType | null>(null)
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (foundCategory) {
        ...
        const subcatRes = await api.getSubcategories(foundCategory.id)
        if (subcatRes.data) {
    ...
    return (
      <main className="min-h-screen pt-[104px] flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-slate-800 mb-4">Ангилал олдсонгүй</h1>
      ...
      <div className="bg-gradient-to-r from-pink-50 to-pink-100/50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          ...
          <nav className="flex items-center gap-2 text-sm mb-4">
            ...
            <span className="text-slate-300">/</span>
            <span className="text-pink-500 font-medium">{category.name}</span>
          </nav>
          ...
          <p className="text-sm text-slate-500 mt-3">{products.length} бүтээгдэхүүн</p>
        </div>
      </div>
